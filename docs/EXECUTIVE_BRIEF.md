...
**Python / FastAPI / Pydantic** · **Ollama** (LLM + embeddings; optional OpenAI/OpenRouter; optional `ollama` Python client for post-decision explanations) · **ChromaDB** (embedded vector store; primary claims index + optional post-decision collection) · **Optional sentence-transformers** (post-decision RAG) · **Optional torch/vision CNN** for damage classification + Grad-CAM · **Minimal web UI** at `/ui`.
